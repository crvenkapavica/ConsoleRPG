...
void GameplayStatics::Initialize(std::vector<std::shared_ptr<Character>>&& InPlayerCharacters, ConsoleMenu& Menu) {
	...
	std::ranges::transform(
		...
		[](auto& Char) { return std::weak_ptr<Character>(Char); }
	);
...
//@TODO: REPLACE ALL "=" with bracket initialization
...
//@TODO: Handle assignment "=" differently?
...
float GameplayStatics::ApplyDamage(const std::weak_ptr<Character>& Instigator, Character* Target, float Damage, const std::unique_ptr<ActiveSpell>& Spell, const bool bIsOnApply) {
	...
	if (bIsOnApply) s << CT << Target->GetAlias() << CL << " suffers from " << CEF << GetEnumString(Spell->GetID()) << CL << " for " << CV << ActualDamage * -1 << CL << " damage [resisted:" << CV << Resisted * -1 << CL << "]\n";
	else s << C << Target->GetAlias() << CL << " suffers from " << CEF << GetEnumString(Spell->GetID()) << CL << " for " << CV << ActualDamage * -1 << CL << " damage [resisted:" << CV << Resisted * -1 << CL << "]\n";
	//==============================================
...
void GameplayStatics::ApplyEffect(const std::shared_ptr<Character>& Instigator, std::vector<std::weak_ptr<Character>>& Targets, std::unique_ptr<ActiveSpell> Spell, const std::optional<ApplyParams>& ApplyParams, const std::optional<EffectParams>& EffectParams) {
	...
	const int Duration = SpellDb::Data[Spell->GetID()][Spell->GetLevel()].Duration;
	const auto Effect = std::make_shared<CombatEffect>(Instigator, Targets, std::move(Spell), ApplyParams, EffectParams, Duration);
...
void GameplayStatics::DisplayLoot(PlayerCharacter& InCharacter, std::vector<std::unique_ptr<Item>>& Loot) {
	...
	std::cout << CLT << GetEnumString(InCharacter.GetClass()) << "'s loot!. (" << Loot.size() << ")" << "\nChoose which items you want to keep.\n";
	while (!Loot.empty()) {
