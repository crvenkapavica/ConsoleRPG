...
void Stoneskin::Apply(const std::shared_ptr<Character>& Instigator, std::vector<std::weak_ptr<Character>>& Targets) {
	RPG_ASSERT(Instigator, "Stoneskin::Apply - Instigator")

	std::vector<std::weak_ptr<Character>> ValidTargets;
	std::vector<CharacterStat> AllyEffectStats;
	ValidTargets.reserve(Targets.empty() ? 1 : Targets.size());
	AllyEffectStats.reserve(Targets.empty() ? 1 : Targets.size());

	const float ApplyMin = SpellDb::Data[ID][Level].ApplyMin;
	const float ApplyMax = SpellDb::Data[ID][Level].ApplyMax;

	auto AddTarget = [&](const std::shared_ptr<Character>& Target) {
		if (!Target || Target->GetTeam() != Instigator->GetTeam())
			return;

		if (std::ranges::any_of(
			AllyEffectStats,
			[&](const CharacterStat& Stat) { return Stat.PtrCharacter == Target.get(); }
		)) return;
		
		ValidTargets.emplace_back(Target);
		AllyEffectStats.emplace_back(CharacterStat{
			.PtrCharacter = Target.get(),
			.StatType = EStatType::ANY,
			.StatMod = EStatMod::ADDITIVE,
			.Stat = &Target->GetArmor().GetActual(),
			.GetDelta = [ApplyMin, ApplyMax](const std::shared_ptr<Character>&) {
				return GameplayStatics::GetRandFloat(ApplyMin, ApplyMax);
			}
		});
	};

	for (const auto& Target : Targets)
		AddTarget(Target.lock());

	if (AllyEffectStats.empty())
		AddTarget(Instigator);

	std::ranges::sort(
		ValidTargets,
		[](const auto& TargetA, const auto& TargetB) {
			const auto SharedA = TargetA.lock();
			const auto SharedB = TargetB.lock();
			return SharedA && SharedB && SharedA->GetAlias() < SharedB->GetAlias();
		}
	);

	Targets = std::move(ValidTargets);

	// ApplyParams has no event gate in the current runner; use EffectParams for the turn-begin armor pulse.
	EffectParams EffParams;
	EffParams.OnEvent = ECombatEvent::ON_TURN_BEGIN;
	EffParams.Flags |= EStructFlags::EFFECT_STAT;
	EffParams.EffectStat = EffectStat(std::move(AllyEffectStats), {});

	auto Spell = std::make_unique<Stoneskin>();
	Spell->SetLevel(Level);
	GameplayStatics::ApplyEffect(Instigator, Targets, std::move(Spell), {}, EffParams);
}
